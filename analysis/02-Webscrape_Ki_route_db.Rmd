...
Need to download and set up a [Docker Selenium image](https://github.com/SeleniumHQ/docker-selenium)
```{bash, engine.opts='-l'}
# For example:
...
write_csv(df, file = file.path(params$results, "KI_Routendatenbank.csv"))
```
